...
void ObjectDetect::process_result(std::shared_ptr<cv::Mat>& frame)
{
    ...
    for (int i = 0, count = (int)*count_data; i < count; ++i) {
        ...
        if (class_id >= 0 && class_id < labels.size() && score >= 0.5f) {
            ...
            cv::rectangle(*frame, cv::Point(topLeft.x, topLeft.y - 40),
                cv::Point(bottomRight.x, topLeft.y), green, cv::FILLED);
            cv::putText(*frame, label, cv::Point(topLeft.x + 10, topLeft.y - 15),
                cv::FONT_HERSHEY_COMPLEX_SMALL, 1, black, 1, cv::LINE_AA);
        }
    }
}
